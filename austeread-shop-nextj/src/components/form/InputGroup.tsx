...
import { lastId } from "@/commons/helpers";
...
export const InputCustom = ({ classParent = "", id, label = "", className = "", error, ...props }: Props) => {
  ...
  const [type, setType] = useState<"text" | "password" | any>(props.type ?? "text");
  return (
    <div className={twMerge("mb-3 last:mb-1", classParent)}>
      ...
      <div className="relative">
        ...
        />
        {props.type == "password" && <ShowEye setType={setType} oldType={props.type} />}
      </div>
...
const ShowEye = (props: { setType: any; oldType: string }) => {
  const [show, setShow] = useState(false);
  const handleShow = (e: React.MouseEvent) => {
    e.preventDefault();
    setShow(!show);
    props.setType(show ? "text" : props.oldType);
  };
