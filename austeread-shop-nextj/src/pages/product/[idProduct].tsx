...
import ProductImageThumbnail from "@/components/product/image-detail-list";
...
const Home: NextPageWithLayout<Props> = ({ product }) => {
  const [sizeSelected, setSizeSelected] = useState<AppInterface.ProductSize>();
  const [keranjangs, setKeranjangs] = useRecoilState(productKeranjangAtom);
  const sizes = Object.values(AppInterface.ProductSize);
  ...
  const handleAddToCar = () => {
    ...
    const krjs = keranjangs.map((ar) => ({ ...ar }));
    const prd = krjs.find((pr) => pr.id == product.id && pr.size == sizeSelected);
    if (!prd) return setKeranjangs([...keranjangs, { ...product, qty, size: sizeSelected }]);
      ...
      <div className="container">
        ...
        <div className="flex p-2 flex-wrap sm:flex-nowrap gap-2 sm:space-x-4">
          <div className="product-image flex gap-2 w-full sm:w-1/2 h-[339px] sm:h-[524px]">
            <div className="h-[339px] sm:h-full flex flex-col overflow-y-auto overflow-x-hidden hide-scroll-bar gap-3 box-border box-content w-20">
              {[1, 2, 3, 4, 5, 6, 7].map((a, i) => (
                <ProductImageThumbnail key={i} />
              ))}
            </div>
          ...
          <div className="product-card-detail text-gray-800 space-y-3 w-full sm:w-1/2">
            ...
            <div className="product-variant  flex gap-2 items-center py-2">
              ...
              <div className="space-x-2 text-sm">
                {sizes.map((sz: AppInterface.ProductSize, i) => {
                  return (
                    <button
                      key={i}
                      className={"border-2 border-dark py-[1px] w-[35px] font-bold" + (sizeSelected == sz ? " bg-dark text-white" : " text-dark")}
                      onClick={(e) => setSizeSelected(sz)}
                    >
                      {sz}
                    </button>
                  );
            ...
            <div className="product-card-action flex justify-center gap-2 flex-col w-full">
              <div className="flex items-center gap-2">
                <span>Quantity : </span>
                <button className="outline-none p-2 font-bold" onClick={() => setQty((q) => (q != 1 ? qty - 1 : 1))}>
                  -
                </button>
              ...
              <div className="w-full">
                <button className="sm:w-auto sm:px-10 w-full font-bold text-base font-GarnettBold py-2 bg-dark text-white" onClick={handleAddToCar}>
                  Add to Cart
                </button>
              </div>
            </div>
